...
class ServerKitAPIClient:
    ...
    def run_algorithm(self, algorithm: str = "rembg", **algo_params) -> List[Tuple]:
        ...
        if response.status_code == 201:
            ...
            print(
                f"Algorithm parameters are not valid!"
            )
            return []
        else:
            print(
    ...
    def get_algorithm_parameters(self, algorithm: str) -> Dict:
        ...
        if response.status_code == 200:
            ...
            print(f"Error status while attempting to get algoritm parameters: {response.status_code}")
            return -1
    ...
    def get_sample_images(self, algorithm: str) -> "np.ndarray":
        ...
        if response.status_code == 200:
            ...
            print(f"Error status while attempting to get algorithm sample images: {response.status_code}")
            return -1
